...
import { RegisterInput, RegisterCompanyInput } from '../auth/validate';
import {
	ConflictError,
	InternalServerError,
	NotFoundError,
...
const registerCompanyAndAdmin = async (
    ...
    logger.info(`Starting company registration for ${data.companyName}, admin ${lowerCaseCompanyEmail}`);
    try {
        const newAdminUser = await prisma.$transaction(async (tx) => {
            ...
            await tx.employee.create({
                data: {
                    ...
                    startDate: new Date(), // Set employment start date
        ...
        try {
            const verificationToken = authUtils.generateEmailVerificationToken(newAdminUser.id, newAdminUser.email);
            ...
            logger.error({ err: emailError, userId: newAdminUser.id }, 'Failed to send verification email after company/admin registration.');
        ...
        logger.info(`Company ${newAdminUser.companyId} and Admin ${newAdminUser.id} registration successful (pending verification).`);
        return newAdminUser; // Return user data (without password)

    } catch (error) {
        logger.error(`Company registration transaction failed for company ${data.companyName}, admin ${lowerCaseCompanyEmail}:`, error);
        if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
            // This might happen if checks fail due to race conditions
            throw new ConflictError('Registration failed: Duplicate company or admin email detected.');
        }
